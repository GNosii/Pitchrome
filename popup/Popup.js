...
function setupUI(exception) {
    chrome.tabs.query({ active: true, currentWindow: true }, (tab) => {
        ...
        if (!isError) {
            ...
            document.getElementById('error-msg').textContent = "Report this on GitHub if it continues! " + exception.name + ': ' + exception.message;
        ...
        if (isDebugBuild) {
            ...
            document.getElementById('debug-ytid').textContent = 'YTID: ' + videoId;
            document.getElementById('debug-thumb').textContent = 'URL Thumbnail: ' + ytThumbnailBaseUrl.replace('$u', videoId);
            document.getElementById('debug-ytunfilteredt').textContent = 'Título no filtrado: ' + tab[0].title;
...
function checkYoutube() {
    chrome.tabs.query({ active: true, currentWindow: true}, (tab) => {
        if ((tab[0].url).indexOf('www.youtube.com/') != -1) {
            isYoutube = true;
            setupUI();
        } else {
            isYoutube = false;
